...
# 把下面改成你的参数设置
org_img_folder = '/data/baisr/bsisr/octnew/test/img'  # 待处理源文件夹路径
tar_img_folder1 = '/data/baisr/bsisr/octnew/test/imgill1'     # 移动到新文件夹路径
tar_img_folder2 = "/data/baisr/bsisr/octnew/test/imgill2"
tar_img_folder3 = "/data/baisr/bsisr/octnew/test/imgill3"
...
for labpath in labellist:
    ...
    if np.all(mask != 0):
        print("not all zero")
        if np.all(mask1 == 0) & np.all(mask2 == 0) & np.all(mask3 == 0):
            print('false')
    # cv2.imwrite(labpath.replace('/test/label', '/test/labelill1'), mask1)
    # cv2.imwrite(labpath.replace('/test/label', '/test/labelill2'), mask2)
    # cv2.imwrite(labpath.replace('/test/label', '/test/labelill3'), mask3)
