...
def dice(im1, im2, empty_score=1.0):
    ...
    intersection = np.dot(im1.flatten(), im2.flatten())
    unionset = np.dot(im1.flatten(), im1.flatten()) + np.dot(im2.flatten(), im2.flatten())
    single_dice_coeff = 2 * float(intersection) / float(unionset + smooth)
    return single_dice_coeff
